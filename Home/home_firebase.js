...
async function registrarAccion() {
    ...
    try {
        ...
        // Llama a la función limpiarCampos() para restablecer el formulario sin recargar la página
        limpiarCampos();
        
        // Vuelve a habilitar los campos y botones para un nuevo registro
        document.querySelector('.noControl').disabled = false;
        document.querySelectorAll('.accion').forEach(button => button.disabled = false);
    } catch (error) {
...
function limpiarCampos() {
    ...
    const noControlField = document.querySelector('.noControl');
    noControlField.value = ''; // Limpia el campo
    noControlField.disabled = false; // Asegúrate de que esté habilitado
    noControlField.style.pointerEvents = 'auto'; // Permite interacción
    noControlField.focus(); // Coloca el cursor en el campo
